...
const (
	...
	jidarFilenameTemplate = "%09d.jdfdb"
	...
	maxJidarFileSize uint32 = 512 * 1024 * 1024 // 512 MiB
...
type jidarStore struct {
	network wire.BitcoinNet

	basePath string

	maxJidarFileSize uint32

	obfMutex         sync.RWMutex
	lruMutex         sync.Mutex
	openJidarsLRU    *list.List

	fileNumToLRUElem map[uint32]*list.Element
	openJidarFiles   map[uint32]*lockableFile
...
// jidarLocation identifies a particular jidar file and location.
type jidarLocation struct {
	jidarFileNum uint32
	fileOffset   uint32
	jidarLen     uint32
}
...
// serializeJidarLoc returns the serialization of the passed jidar location.
// This is data to be stored into the jidar index metadata for each jidar.
func serializeJidarLoc(loc jidarLocation) []byte {
	// The serialized jidar location format is:
	//
	//  [0:4]  Jidar file (4 bytes)
	//  [4:8]  File offset (4 bytes)
	//  [8:12] Jidar length (4 bytes)
	var serializedData [12]byte
	byteOrder.PutUint32(serializedData[0:4], loc.jidarFileNum)
	byteOrder.PutUint32(serializedData[4:8], loc.fileOffset)
	byteOrder.PutUint32(serializedData[8:12], loc.jidarLen)
	return serializedData[:]
}
...
// jidarFilePath return the file path for the provided jidar file number.
func jidarFilePath(dbPath string, fileNum uint32) string {
	fileName := fmt.Sprintf(jidarFilenameTemplate, fileNum)
	return filepath.Join(dbPath, fileName)
}
...
// this function does not keep track of the open file and it is not subject to
// the maxOpenFiles limit.
func (s *jidarStore) openWriteFile(fileNum uint32) (filer, error) {
	// The current jidar file needs to be read-write so it is possible to
	// append to it.  Also, it shouldn't be part of the least recently used
	// file.
	filePath := jidarFilePath(s.basePath, fileNum)
	file, err := os.OpenFile(filePath, os.O_RDWR|os.O_CREATE, 0666)
	if err != nil {
		str := fmt.Sprintf("failed to open file %q: %v", filePath, err)
		return nil, makeDbErr(database.ErrDriverSpecific, str, err)
...
// This function MUST be called with the overall files mutex (s.obfMutex) locked
// for WRITES.
func (s *jidarStore) openFile(fileNum uint32) (*lockableFile, error) {
	// Open the appropriate file as read-only.
	filePath := jidarFilePath(s.basePath, fileNum)
	file, err := os.Open(filePath)
	if err != nil {
	...
	jidarFile := &lockableFile{file: file}
	...
	// therefore should be closed last.
	s.lruMutex.Lock()
	lruList := s.openJidarsLRU
	if lruList.Len() >= maxOpenFiles {
		lruFileNum := lruList.Remove(lruList.Back()).(uint32)
		oldJidarFile := s.openJidarFiles[lruFileNum]

		// Close the old file under the write lock for the file in case
		// any readers are currently reading from it so it's not closed
		// out from under them.
		oldJidarFile.Lock()
		_ = oldJidarFile.file.Close()
		oldJidarFile.Unlock()

		delete(s.openJidarFiles, lruFileNum)
		delete(s.fileNumToLRUElem, lruFileNum)
	}
	s.fileNumToLRUElem[fileNum] = lruList.PushFront(fileNum)
	s.lruMutex.Unlock()

	// Store a reference to it in the open jidar files map.
	s.openJidarFiles[fileNum] = jidarFile

	return jidarFile, nil
}
...
// deleteFile removes the jidar file for the passed flat file number.  The file
// must already be closed and it is the responsibility of the caller to do any
// other state cleanup necessary.
func (s *jidarStore) deleteFile(fileNum uint32) error {
	filePath := jidarFilePath(s.basePath, fileNum)
	if err := os.Remove(filePath); err != nil {
		return makeDbErr(database.ErrDriverSpecific, err.Error(), err)
	}
...
// writeData is a helper function for writeJidar which writes the provided data
// at the current write offset and updates the write cursor accordingly.  The
// field name parameter is only used when there is an error to provide a nicer
// error message.
//
...
// held and must only be called during a write transaction so it is effectively
// locked for writes.  Also, the write cursor current file must NOT be nil.
func (s *jidarStore) writeData(data []byte, fieldName string) error {
	wc := s.writeCursor
	n, err := wc.curFile.file.WriteAt(data, int64(wc.curOffset))
	wc.curOffset += uint32(n)
	if err != nil {
...
// writeJidar appends the specified raw jidar bytes to the store's write cursor
// location and increments it accordingly.  When the jidar would exceed the max
// file size for the current flat file, this function will close the current
// file, create the next file, update the write cursor, and write the jidar to
// the new file.
//
// The write cursor will also be advanced the number of bytes actually written
// in the event of failure.
//
// Format: <network><jidar length><serialized jidar><checksum>
func (s *jidarStore) writeJidar(rawJidar []byte) (jidarLocation, error) {
	// Compute how many bytes will be written.
	// 4 bytes each for jidar network + 4 bytes for jidar length +
	// length of raw jidar + 4 bytes for checksum.
	jidarLen := uint32(len(rawJidar))
	fullLen := jidarLen + 12

	// Move to the next jidar file if adding the new jidar would exceed the
	// max allowed size for the current jidar file.  Also detect overflow
	// to be paranoid, even though it isn't possible currently, numbers
	// might change in the future to make it possible.
	//
	// NOTE: The writeCursor.offset field isn't protected by the mutex
	...
	wc := s.writeCursor
	finalOffset := wc.curOffset + fullLen
	if finalOffset < wc.curOffset || finalOffset > s.maxJidarFileSize {
		// This is done under the write cursor lock since the curFileNum
		// field is accessed elsewhere by readers.
		//
		// Close the current write file to force a read-only reopen
	...
	// Jidar length.
	byteOrder.PutUint32(scratch[:], jidarLen)
	if err := s.writeData(scratch[:], "jidar length"); err != nil {
		return jidarLocation{}, err
	}
	_, _ = hasher.Write(scratch[:])

	// Serialized jidar.
	if err := s.writeData(rawJidar[:], "jidar"); err != nil {
		return jidarLocation{}, err
	}
	_, _ = hasher.Write(rawJidar)
...
// handleRollback rolls the jidar files on disk back to the provided file number
// and offset.  This involves potentially deleting and truncating the files that
// were partially written.
//
// There are effectively two scenarios to consider here:
...
//   2) More permanent failures such as hard disk death and/or removal
//
// In either case, the write cursor will be repositioned to the old jidar file
// offset regardless of any other errors that occur while attempting to undo
// writes.
//
// For the first scenario, this will lead to any data which failed to be undone
...
//
// For the second scenario, the metadata which stores the current write cursor
// position within the jidar files will not have been updated yet and thus if
// the system eventually recovers (perhaps the hard drive is reconnected), it
// will also lead to any data which failed to be undone being overwritten and
// thus behaves as desired.
//
// Therefore, any errors are simply logged at a warning level rather than being
// returned since there is nothing more that could be done about it anyways.
func (s *jidarStore) handleRollback(oldJidarFileNum, oldJidarOffset uint32) {
	// Grab the write cursor mutex since it is modified throughout this
	// function.
	wc := s.writeCursor
	wc.Lock()
	...
	// Nothing to do if the rollback point is the same as the current write
	// cursor.
	if wc.curFileNum == oldJidarFileNum && wc.curOffset == oldJidarOffset {
		return
	}

	// Regardless of any failures that happen below, reposition the write
	// cursor to the old jidar file and offset.
	defer func() {
		wc.curFileNum = oldJidarFileNum
		wc.curOffset = oldJidarOffset
	}()

	log.Debugf("ROLLBACK: Rolling back to jidar file %d, offset %d",
		oldJidarFileNum, oldJidarOffset)

	// Close the current write file if it needs to be deleted.  Then delete
	// all files that are newer than the provided rollback file while
	// also moving the write cursor file backwards accordingly.
	if wc.curFileNum > oldJidarFileNum {
		wc.curFile.Lock()
		if wc.curFile.file != nil {
			_ = wc.curFile.file.Close()
			wc.curFile.file = nil
	...
	for ; wc.curFileNum > oldJidarFileNum; wc.curFileNum-- {
		if err := s.deleteFileFunc(wc.curFileNum); err != nil {
			log.Warnf("ROLLBACK: Failed to delete jidar file "+
				"number %d: %v", wc.curFileNum, err)
			return
		}
	}
	...
	// Truncate the to the provided rollback offset.
	if err := wc.curFile.file.Truncate(int64(oldJidarOffset)); err != nil {
		wc.curFile.Unlock()
		log.Warnf("ROLLBACK: Failed to truncate file %d: %v",
			wc.curFileNum, err)
		return
...
// scanJidarFiles searches the database directory for all flat jidar files to
// find the end of the most recent file.  This position is considered the
// current write cursor which is also stored in the metadata.  Thus, it is used
// to detect unexpected shutdowns in the middle of writes so the jidar files
// can be reconciled.
func scanJidarFiles(dbPath string) (int, uint32) {
	lastFile := -1
	fileLen := uint32(0)
	for i := 0; ; i++ {
		filePath := jidarFilePath(dbPath, uint32(i))
		st, err := os.Stat(filePath)
		if err != nil {
			break
		}
	...
	log.Tracef("Scan found latest jidar file #%d with length %d", lastFile,
		fileLen)
	return lastFile, fileLen
}

// newJidarStore returns a new jidar store with the current jidar file number
// and offset set and all fields initialized.
func newJidarStore(basePath string, network wire.BitcoinNet) *jidarStore {
	// Look for the end of the latest jidar to file to determine what the
	// write cursor position is from the viewpoint of the jidar files on
	// disk.
	fileNum, fileOff := scanJidarFiles(basePath)
	if fileNum == -1 {
		fileNum = 0
		fileOff = 0
	}
	...
	store := &jidarStore{
		network:          network,
		basePath:         basePath,
		maxJidarFileSize: maxJidarFileSize,
		openJidarFiles:   make(map[uint32]*lockableFile),
		openJidarsLRU:    list.New(),
		fileNumToLRUElem: make(map[uint32]*list.Element),
