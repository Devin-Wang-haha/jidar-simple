...
var (
	...
	// jidarIdxBucketID is the ID of the internal jidar metadata bucket.
	// It is the value 2 encoded as an unsigned big-endian uint32.
	jidarIdxBucketID = [4]byte{0x00, 0x00, 0x00, 0x02}

	// blockIdxBucketName is the bucket used internally to track block
	// metadata.
	blockIdxBucketName = []byte("ffldb-blockidx")

	// jidarIdxBucketName is the bucket used internally to track jidar
	// metadata.
	jidarIdxBucketName = []byte("ffldb-jidaridx")

	// writeLocKeyName is the key used to store the current write file
	// location.
	writeLocKeyName = []byte("ffldb-writeloc")

	// writeLocJidarKeyName is the key used to store the current write file
	// location.
	writeJidarLocKeyName = []byte("ffldb-writeJidarloc")
)
...
// pendingJidar houses a jidarBlock that will be written to disk when the database
// transaction is commited
type pendingJidar struct{
	hash *chainhash.Hash
	bytes []byte
}
...
type transaction struct {
	...
	jidarIdxBucket *bucket			// The jidar index bucket.
	...
	// JidarBlocks that need to stored on commit. The pendingJidars map is
	// kept to allow quick looups of pending data by block hash
	pendingJidars map[chainhash.Hash]int
	pendingJidarData []pendingJidar
...
func (tx *transaction) StoreBlock(block *btcutil.Block) error {
	...
	// Generate the MerkleBranch
	posBits := block.GeneratePosBits()
	jidarBlock := btcutil.MakeJidarBlock(block, posBits)

	branchBytes, err := jidarBlock.Bytes()
	if err != nil {
		str := fmt.Sprintf("failed to get serialized bytes for jidarBlock %s",
			blockHash)
		return makeDbErr(database.ErrDriverSpecific, str, err)
	}

	// Add the jidarBlock to be stored to the list of pending jidarBlocks to store
	// when the transaction is commtted. Also, add it to pending jidarBlocks
	// map so it is easy to determine the jidarBlock is pending based on the
	// block hash
	if tx.pendingJidars == nil {
		tx.pendingJidars = make(map[chainhash.Hash]int)
	}
	tx.pendingJidars[*blockHash] = len(tx.pendingBlockData)
	tx.pendingJidarData = append(tx.pendingJidarData, pendingJidar{
		hash: blockHash,
		bytes: branchBytes,
	})
	log.Tracef("Added block %s to pending jidarblocks", blockHash)
...
// fetchJidarRow fetches the metadata stored in the jidar index for the provided
// hash.  It will return ErrJidarNotFound if there is no entry.
func (tx *transaction) fetchJidarRow(hash *chainhash.Hash) ([]byte, error) {
	jidarRow := tx.jidarIdxBucket.Get(hash[:])
	if jidarRow == nil {
		str := fmt.Sprintf("jidar %s does not exist", hash)
		return nil, makeDbErr(database.ErrJidarNotFound, str, nil)
	}
...
func (tx *transaction) writePendingAndCommit() error {
	...
	// Save the current jiadr store write position for potential rollback.
	// These variables are only updated here in this function and there can
	// only be one write transaction active at a time, so it's safe to store
	// them for potential rollback.
	wcJidar := tx.db.jidar.writeCursor
	wcJidar.RLock()
	oldJidFileNum := wcJidar.curFileNum
	oldJidOffset := wcJidar.curOffset
	wcJidar.RUnlock()
	...
	rollback := func() {
		...
		// Rollback any modifications made to the jidar files if needed.
		tx.db.jidar.handleRollback(oldJidFileNum, oldJidOffset)
	...
	// Look through all of the pending jidarblocks to store and write them
	for _, jidarData := range tx.pendingJidarData {
		log.Tracef("Storing jidarblock %s", jidarData.hash)
		location, err := tx.db.jidar.writeJidar(jidarData.bytes)
		if err != nil {
			rollback()
			return err
		}

		// Add a record in the jidar index for the jidarblock.  The record
		// includes the location information needed to locate the jidarblock
		// on the filesystem as well as the jidarblock header since they are
		// so commonly needed.
		jidarRow := serializeJidarLoc(location)
		err = tx.jidarIdxBucket.Put(jidarData.hash[:], jidarRow)
		if err != nil {
			rollback()
			return err
		}
	...
	// Update the metadata for the current write file and offset of jidar.
	writeJidarRow := serializeWriteRow(wcJidar.curFileNum, wcJidar.curOffset)
	if err := tx.metaBucket.Put(writeJidarLocKeyName, writeJidarRow); err != nil {
		rollback()
		return convertErr("failed to store write cursor of jidar", err)
	}
...
func (db *db) begin(writable bool) (*transaction, error) {
	...
	tx.jidarIdxBucket = &bucket{tx:tx, id: jidarIdxBucketID}
	return tx, nil
}
...
func initDB(ldb *leveldb.DB) error {
	...
	// The starting jidar file write cursor location is file num 0, offset
	// 0.
	batch.Put(bucketizedKey(metadataBucketID, writeJidarLocKeyName),
		serializeWriteRow(0, 0))
	...
	// Create jidar index bucket and set the current bucket id.
	//
	// NOTE: Since buckets are virtualized through the use of prefixes,
	// there is no need to store the bucket index data for the metadata
	// bucket in the database.  However, the first bucket ID to use does
	// need to account for it to ensure there are no key collisions.
	batch.Put(bucketIndexKey(metadataBucketID, jidarIdxBucketName),
		jidarIdxBucketID[:])
	batch.Put(curBucketIDKeyName, jidarIdxBucketID[:])
...
func openDB(dbPath string, network wire.BitcoinNet, create bool) (database.DB, error) {
	...
	jidar := newJidarStore(dbPath, network)
	cache := newDbCache(ldb, store, defaultCacheSize, defaultFlushSecs)
	pdb := &db{store: store, jidar:jidar, cache: cache}
